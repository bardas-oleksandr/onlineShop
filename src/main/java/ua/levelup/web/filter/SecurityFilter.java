...
//                if (shouldBeProtectedFromBlocked(url) && User.UserState.BLOCKED.ordinal() == user.getState()) {
//                    logger.info("URL " + url + " was protected from blocked user");
//                    httpServletResponse.sendRedirect(CONTEXT + PROFILE_JSP);
//                } else if (shouldBeProtectedFromActive(url) && User.UserState.ACTIVE.ordinal() == user.getState()) {
//                    logger.info("URL " + url + " was protected from user");
//                    httpServletResponse.sendRedirect(CONTEXT);
//                } else if (shouldBeProtectedFromAdmin(url) && User.UserState.ADMIN.ordinal() == user.getState()) {
