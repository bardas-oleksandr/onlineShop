...
public class CategoryRestControllerIntegrationTest {
    ...
    * */
    @Test
    @Sql({"classpath:schema_clean.sql", "classpath:schema_insert_categoryTest.sql"})
    public void deleteTest_givenJSONAcceptAndJSONContentTypeAndCorrectId_whenGet_thenNoContent()
            throws Exception {
        int id = 20;
        given()
                .log().all()
    ...
    * */
    @Test
    @Sql({"classpath:schema_clean.sql", "classpath:schema_insert_categoryTest.sql"})
    public void deleteTest_givenJSONAcceptAndJSONContentTypeAndNotCorrectId_whenGet_thenNotFound()
            throws Exception {
        int id = 999;
        given()
                .log().all()
    ...
    * */
    @Test
    @Sql({"classpath:schema_clean.sql", "classpath:schema_insert_categoryTest.sql"})
    public void deleteTest_givenTextPlainAcceptAndJSONContentType_whenGet_thenNotAcceptable()
            throws Exception {
        int id = 1;
        given()
                .log().all()
    ...
    * */
    @Test
    @Sql({"classpath:schema_clean.sql", "classpath:schema_insert_categoryTest.sql"})
    public void deleteTest_givenJSONAcceptAndNoJSONContentType_whenGet_thenUnsupported()
            throws Exception {
        int id = 1;
        given()
                .log().all()
    ...
    * */
    @Test
    @Sql({"classpath:schema_clean.sql", "classpath:schema_insert_categoryTest.sql"})
    public void deleteTest_givenTextPlainAcceptAndNoJSONContentType_whenGet_thenUnsupported()
            throws Exception {
        int id = 1;
        given()
                .log().all()
...
//    }
    ...
    /*Сценарий: получение списка категорий товаров заданного уровня.
    *           Категории такого уровня есть в БД.
    *   Дано:
    *       -   level = 1
    *       -   Accept = "text/plain"
    *       -   Content-Type = "application/json;charset=UTF-8"
